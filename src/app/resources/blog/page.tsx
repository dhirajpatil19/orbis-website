    ...
    <>
      <section className="relative bg-brand-900 py-16 sm:py-24 overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-4 relative">
          <p className="inline-flex items-center gap-2 text-accent-400 font-bold text-xs sm:text-sm uppercase tracking-[0.2em] mb-4">
            <span className="h-1.5 w-1.5 rounded-full bg-accent-500" /> Resources
          </p>
          <h1 className="text-white font-display text-4xl sm:text-5xl font-semibold max-w-2xl leading-tight">The Orbis Blog</h1>
          <p className="mt-4 text-brand-100/85 max-w-xl text-base sm:text-lg">Insights for parents on admissions, future skills and childhood learning.</p>
        </div>
      </section>
      ...
      <section className="py-16 sm:py-24">
        <div className="mx-auto max-w-7xl px-4">
          ...
          {featured && (
            <Reveal>
              <Link href={`/resources/blog/${featured.slug}`} className="group grid lg:grid-cols-2 gap-0 rounded-[2rem] overflow-hidden bg-white border border-brand-100 shadow-sm hover:shadow-2xl hover:shadow-brand-900/10 transition-all mb-10">
                ...
                <div className="p-8 sm:p-10 flex flex-col justify-center">
                  <p className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-accent-600 mb-3">
                    <CalendarDays className="h-4 w-4" /> {featured.date} · Latest
                  </p>
                  <h2 className="font-display text-2xl sm:text-3xl font-semibold text-ink-900 leading-tight mb-4 group-hover:text-brand-700 transition-colors">{featured.title}</h2>
              ...
              <Reveal key={p.slug} delay={i * 80}>
                <Link href={`/resources/blog/${p.slug}`} className="group block rounded-[2rem] bg-white border border-brand-100 shadow-sm hover:shadow-2xl hover:shadow-brand-900/10 transition-all hover:-translate-y-1.5 overflow-hidden h-full">
                  ...
                  <div className="p-6">
                    ...
                    <h2 className="font-display text-lg font-semibold text-ink-900 leading-snug mb-2 group-hover:text-brand-700 transition-colors line-clamp-2">{p.title}</h2>
